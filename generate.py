...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--pipeline_type', type=str, default="auto", choices=["sd", "sdxl", "sld", "sega", "auto"])
...
def validate(
    ...
    # Auto-detect pipeline type if needed
    if args.pipeline_type == "auto":
    ...
    pipeline_cls = PIPELINES[pipeline_type]

    # Handle SDXL vs SD pipeline creation
    ...
    for i in range(num_total_iters):
        ...
        for j in range(start_idx, end_idx):
            ...
            # Adjust latent dimensions for SDXL vs SD
            is_sdxl = is_sdxl_model(args.pretrained_model_name_or_path)
            latent_size = 128 if is_sdxl else 64
            all_latents.append(torch.randn(1, 4, latent_size, latent_size, generator=generator, device=device, dtype=weight_dtype))
...
def main():
    ...
    # Load second text encoder and tokenizer for SDXL
    text_encoder_2 = None
    tokenizer_2 = None
    if sdxl_model:
