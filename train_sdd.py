...
def validate(
    ...
    # Use SDXL pipeline if it's an SDXL model
    if is_sdxl_model(args.pretrained_model_name_or_path):
...
def encode_prompt(
    ...
    def encode_with_text_encoder(input_ids, attention_mask, text_enc, use_pooled=False):
        outputs = text_enc(input_ids=input_ids.to(device), attention_mask=attention_mask.to(device) if use_attention_mask else None)
        if use_pooled and hasattr(outputs, 'pooler_output'):
    ...
    if is_sdxl:
        ...
        # First text encoder
        uncond_emb_1 = encode_with_text_encoder(uncond_input["input_ids"], uncond_input["attention_mask"], text_encoder)
